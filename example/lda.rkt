...
(define-constant ALPHA (Integer-type) 1)
...
(define-incremental num-for-topic-text (Vector-type Topic (Vector-type WordText (Integer-type))) (word->topic) ()
...
(define-incremental num-for-topic (Vector-type Topic (Integer-type)) (word->topic) ()
  (build-vector
   NUM_TOPICS
   (lambda (topic)
     (vector-sum (vector-ref num-for-topic-text topic)))))

(define-incremental num-for-document-topic (Vector-type Document (Vector-type Topic (Integer-type))) (word->topic) ()
...
(define (main)
  (for ([iter (in-range 1000)])
    (for ([w (in-range NUM_WORD_INSTANCES)])
      (assign-word->topic! w (sample (compute-unnormalized-distribution w)))))
